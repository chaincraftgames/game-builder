...
export function executeChanges(model: ModelWithOptions) {
  return async (state: RuntimeStateType): Promise<Partial<RuntimeStateType>> => {
    ...
    let llmResponse: { publicMessage?: string; privateMessages?: Record<string, string>; imagePrompt?: string; rationale?: string; stateDelta?: any[] } = {};
    let updatedState: any;

    if (generatedFunctionBody) {
      ...
      // Build callLLM closure with narrative context from the expanded instructions
      // (instructions already have narrative markers expanded at the top of execute-changes)
      // TODO: Replace regex extraction with explicit narrativeKeys from planner output
      // (add narrativeKeys: string[] to AutomaticTransitionHintSchema, pass through to runtime)
      const narrativeSections: string[] = [];
      const narrativeRegex = /!___ NARRATIVE_START:\w+ ___!\n\n([\s\S]*?)\n\n!___ NARRATIVE_END:\w+ ___!/g;
      let match;
      while ((match = narrativeRegex.exec(instructions)) !== null) {
        narrativeSections.push(match[1]);
      }
      ...
      // Execute in sandbox
      const partialUpdate = await executeMechanic(generatedFunctionBody, flatState, callLLM);
