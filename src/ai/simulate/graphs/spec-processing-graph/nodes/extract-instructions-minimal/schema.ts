/**
 * Minimal Planner Schemas for Instructions Extraction
 * 
...
export const PlayerActionHintMinimalSchema = z.object({
...
export const AutomaticTransitionHintMinimalSchema = z.object({
...
export const PhaseInstructionsHintMinimalSchema = z.object({
  phase: z.string().describe("Phase identifier (must require player input)"),
  playerActions: z.array(PlayerActionHintMinimalSchema).describe("Player actions available in this phase"),
  phaseSummary: z.string().max(300).describe("Brief summary of what player input is needed"),
});

export const InstructionsPlanningResponseMinimalSchema = z.object({
  naturalLanguageSummary: z.string().describe("1-2 sentence summary of instruction structure"),
  playerPhases: z.array(PhaseInstructionsHintMinimalSchema).describe("Hints ONLY for phases requiring player input"),
  transitions: z.array(AutomaticTransitionHintMinimalSchema).describe("Hints for EACH automatic transition"),
  globalNotes: z.array(z.string()).optional().describe("Cross-cutting game rules (optional)"),
});

// Export types
export type PlayerActionHintMinimal = z.infer<typeof PlayerActionHintMinimalSchema>;
export type AutomaticTransitionHintMinimal = z.infer<typeof AutomaticTransitionHintMinimalSchema>;
export type PhaseInstructionsHintMinimal = z.infer<typeof PhaseInstructionsHintMinimalSchema>;
export type InstructionsPlanningResponseMinimal = z.infer<typeof InstructionsPlanningResponseMinimalSchema>;

// JSON schema exports for prompts
export const InstructionsPlanningResponseMinimalSchemaJson = zodToJsonSchema(
