...
- **Validation enum casing**: String literals in \`validation.checks\` allowed-value arrays MUST use the exact same casing as the corresponding field's \`enumValues\` in the state schema. If the schema defines \`["rock", "paper", "scissors"]\` (lowercase), the validation check MUST also use lowercase. Mismatched casing causes every submission to be rejected before the mechanic runs.
- **No timeout or missing-submission fallback in mechanicsGuidance**: The runtime has no concept of timeouts or time limits. When all players have submitted (guaranteed by \`allPlayersCompletedActions\` precondition), every player's \`currentAction\` will be non-null. Do NOT include instructions like "if a player has no submitted currentAction (timeout), assign a random choice" — this language will be generated into mechanic code that adds unreachable fallback branches and implies the platform supports timeouts, which it does not. Always write mechanicsGuidance as if all required submissions are present.
