...
export const createBasicTests = (functionNames: string[], functionDesign?: FunctionDesign): string => {
  ...
  const functionTests = functionNames.map(fnName => {
    ...
    const designDoc = functionDesign?.functions.find((fn: any) => fn.name === fnName);
...
export const generateImprovedBlackBoxTests = async (
  ...
  if (functionDesign) {
    functionNames = functionDesign.functions.map((fn: any) => fn.name);
    ...
    ${functionDesign.functions.map((fn: any) => `
    ${fn.name}:
