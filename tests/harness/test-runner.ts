...
import {
  runReliabilityTest as runReliabilityTestInternal,
...
export async function runGameTestScenario(
  ...
  if (scenarioIndex >= gameTest.scenarios.length) {
    throw new Error(
      `Scenario index ${scenarioIndex} out of range for ${gameKey} (has ${gameTest.scenarios.length} scenarios)`,
    );
  }
  ...
  console.log(
    `Game ID: ${displayGameId} ${usingExistingArtifacts ? "(reusing existing artifacts)" : "(generating fresh artifacts)"}`,
  );
  ...
  try {
    ...
    const result = await executeGameTest(
      gameTest,
      scenario,
      gameIdForReuse,
      getGameTestsDirectory(),
    );
