...
async function generateNarrative(
  ...
  const prompt = `!___ CACHE:static-instructions ___!
You are filling in narrative guidance sections for a game specification.
...
SKELETON SPECIFICATION (CONSTANT):
${skeletonSpec}
!___ END-CACHE ___!

PREVIOUSLY GENERATED NARRATIVES:
${previousNarratives.map(n => `<!-- NARRATIVE:${n.key} -->
${n.content}`).join('\n\n')}

CURRENT SPEC WITH INJECTED NARRATIVES:
${specWithPlaceholder}
